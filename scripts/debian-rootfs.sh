...
if [ ! -e  ${STAMPS}/debian-rootfs.bootstrapped -o ${FORCE_BUILD} -eq 1 ]; then
	...
	zotac | qemu)
		sudo debootstrap testing ${STAGING} http://ftp.debian.org/debian || exit 1
	;;
	arm-chroot)
		sudo debootstrap --arch=armel --foreign testing ${STAGING} http://http.debian.net/debian || exit 1
		...
		sudo echo "deb http://ftp.debian.org/debian testing main" > ${STAGING}/etc/apt/sources.list
	;;
